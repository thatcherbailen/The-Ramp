...
const PHASE_PALETTE = [
  { pill: { bg: '#FBE7E0', color: '#C24A24' }, fill: '#F5552E' },
    ...
    <Modal title={initial ? 'Edit task' : 'Add task'} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 150px', gap: 14 }}>
          ...
          <div><label className="form-label" style={{ color: '#C24A24' }}>Due → calendar</label><input className="form-input" type="date" value={f.due || ''} onChange={e => upd({ due: e.target.value })} /></div>
        </div>
    ...
    <Modal title={initial ? 'Edit goal' : 'New goal'} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 56px', gap: 14, alignItems: 'end' }}>
          ...
          <div><label className="form-label">Colour</label><input type="color" value={f.color || '#F5552E'} onChange={e => setF(v => ({ ...v, color: e.target.value }))} style={{ width: '100%', height: 44, border: '1px solid #E4DFD8', borderRadius: 12, background: '#fff', cursor: 'pointer', padding: 4 }} /></div>
        </div>
        {!initial && <div style={{ fontSize: 12, color: '#9C958B', lineHeight: 1.5 }}>Your goal starts with one phase — add and edit phases from “Edit phases” once it’s created.</div>}
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
...
function PhaseEditor({ goal, onClose }: { goal: Goal; onClose: () => void }) {
  ...
  return (
    <Modal title="Edit phases" onClose={onClose} width={600}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ fontSize: 12, color: '#9C958B', lineHeight: 1.5 }}>Define each stage of “{goal.name}” — name it, say what it means, and set its dates. Tasks are grouped under these phases.</div>
        {phases.map((p, i) => (
          <div key={p.id} style={{ border: '1px solid #ECE8E2', borderRadius: 14, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button onClick={() => removeP(p.id)} title="Remove phase" style={{ width: 36, height: 36, borderRadius: 10, border: '1px solid #F0CFC6', background: '#fff', color: '#D8431F', cursor: 'pointer', flexShrink: 0 }}>✕</button>
            </div>
        ...
        <button onClick={addP} style={{ padding: '10px 0', borderRadius: 12, border: '1px dashed #D8D2C8', background: '#fff', color: '#9C958B', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>＋ Add phase</button>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
          <button onClick={onClose} style={cancelBtn}>Cancel</button>
...
export default function TasksPage() {
  ...
  return (
    <div>
      {/* Goal switcher */}
      <div className="scroll-x" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 18 }}>
        {goals.map(g => {
          ...
          return (
            <button key={g.id} onClick={() => { setGoalId(g.id); setPhaseFilter('all'); }} style={{
              ...
              borderColor: active ? '#1A1613' : '#ECE8E2', background: active ? '#1A1613' : '#fff', color: active ? '#fff' : '#6B655E',
        ...
        <button onClick={() => setGoalModal({ mode: 'new' })} style={{ padding: '8px 14px', borderRadius: 999, border: '1px dashed #D8D2C8', background: '#fff', color: '#9C958B', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap' }}>＋ New goal</button>
      </div>

      {/* Header */}
      <div className="page-head" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 500, color: '#9C958B' }}>
            {goal.name}{goal.targetDate ? ` · target ${fmtD(goal.targetDate)}` : ''}
            <button onClick={() => setGoalModal({ mode: 'edit' })} title="Edit goal" style={{ border: 'none', background: 'none', color: '#C5BFB6', cursor: 'pointer', padding: 0, display: 'flex' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M4 20h4l10-10-4-4L4 16v4z" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round"/></svg>
            </button>
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'stretch', gap: 12, marginBottom: 22, flexWrap: 'wrap' }}>
            ...
            return (
              <div key={p.id} className="card" style={{ padding: '14px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 9 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#9C958B' }}>{d}/{tot} · {pct}%</span>
                </div>
        ...
        <button onClick={() => setPhaseOpen(true)} style={{ alignSelf: 'center', padding: '10px 16px', borderRadius: 12, border: '1px solid #E4DFD8', background: '#fff', color: '#6B655E', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap' }}>Edit phases</button>
      </div>

      {/* Filter bar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 18 }}>
        <div className="scroll-x" style={{ display: 'flex', gap: 3, background: '#F4F1EC', borderRadius: 12, padding: 3 }}>
          {['all', ...goal.phases.map(p => p.id)].map((pid, i) => {
            ...
            return (
              <button key={pid} onClick={() => setPhaseFilter(pid)} title={pid === 'all' ? 'All phases' : goal.phases[i - 1]?.name} style={{
                ...
                background: active ? '#F5552E' : 'none', color: active ? '#fff' : '#9C958B', whiteSpace: 'nowrap',
              }}>{label}</button>
            );
          })}
        </div>
          ...
          {(['High', 'Medium', 'Low'] as Priority[]).map(p => (
            <button key={p} onClick={() => setPriFilter(prev => { const s = new Set(prev); s.has(p) ? s.delete(p) : s.add(p); return s; })}
              style={{ padding: '7px 12px', borderRadius: 10, border: '1px solid', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, fontWeight: 700,
                borderColor: priFilter.has(p) ? PRI_COLORS[p] : '#E4DFD8', background: priFilter.has(p) ? PRI_COLORS[p] + '22' : '#fff', color: priFilter.has(p) ? PRI_COLORS[p] : '#9C958B' }}>{p === 'Medium' ? 'Med' : p}</button>
          ))}
        </div>
        <button onClick={() => setShowDone(v => !v)} style={{ padding: '7px 14px', borderRadius: 10, border: '1px solid', borderColor: showDone ? '#1A1613' : '#E4DFD8', background: showDone ? '#1A1613' : '#fff', color: showDone ? '#fff' : '#9C958B', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, fontWeight: 600 }}>✓ Completed</button>
      ...
      {all.length === 0 ? (
        <div className="card" style={{ padding: '48px 40px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontSize: 14, color: '#9C958B', maxWidth: 360, lineHeight: 1.5 }}>Add tasks and assign them to a phase. Set due dates and they land on your calendar.</div>
            ...
            <div key={sec.phase.id}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 11, borderBottom: '1px solid #ECE8E2', marginBottom: 14 }}>
                ...
                <div>
                  ...
                  {(sec.phase.description || sec.phase.startDate) && (
                    <div style={{ fontSize: 12, fontWeight: 500, color: '#9C958B', marginTop: 1 }}>
              ...
              {sec.weeks.map(wk => (
                <div key={wk.label} style={{ marginBottom: 16 }}>
                  {wk.label && wk.label !== 'Tasks' && <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#B5AEA4', marginBottom: 9 }}>{wk.label}</div>}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                    {wk.tasks.map(t => (
                      <div key={t.id} onClick={() => { toggle(t.id); setTick(x => x + 1); }} style={{
                        ...
                        <span style={{ width: 20, height: 20, borderRadius: 6, border: '2px solid', borderColor: done[t.id] ? '#F5552E' : '#D6D0C8', background: done[t.id] ? '#F5552E' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
...
const cancelBtn: React.CSSProperties = { padding: '11px 22px', borderRadius: 12, border: '1px solid #E4DFD8', background: '#fff', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 600 };
