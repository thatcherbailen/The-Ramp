    ...
    <div>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:22 }}>
        {allTags.map(tag => (
          <button
            ...
            style={{ padding:'6px 14px', borderRadius:999, border:`1px solid ${activeTag===tag ? '#F5552E' : '#ECE8E2'}`, background: activeTag===tag ? '#FBE7E0' : '#fff', color: activeTag===tag ? '#C24A24' : '#6B655E', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit', letterSpacing:'.03em' }}
          >{tag}</button>
        ))}
      </div>
      ...
      {loading && items.length === 0 ? (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {[...Array(6)].map((_,i) => (
            <div key={i} className="card" style={{ padding:'20px 22px', height:120, background:'linear-gradient(90deg,#F4F1EC 0%,#FBF9F6 50%,#F4F1EC 100%)', backgroundSize:'200% 100%', animation:'shimmer 1.4s infinite' }} />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="card" style={{ padding:'56px 40px', textAlign:'center' }}>
          ...
          <div style={{ fontSize:14, color:'#9C958B' }}>Hit Refresh to pull today&apos;s tech headlines.</div>
        </div>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {filtered.map(item => (
              ...
              <div style={{ display:'flex', alignItems:'flex-start', gap:14 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, flexWrap:'wrap' }}>
                    <span style={{ fontSize:10, fontWeight:700, letterSpacing:'.07em', textTransform:'uppercase', color:'#C24A24', background:'#FBE7E0', padding:'3px 10px', borderRadius:999 }}>{item.category}</span>
                    <span style={{ fontSize:11, color:'#B5AEA4', fontWeight:500 }}>{item.source}</span>
                  </div>
                  <a
                    href={item.url}
                    target="_blank"
                  ...
                  >
                    <div style={{ fontWeight:700, fontSize:16, lineHeight:1.35, letterSpacing:'-.01em', marginBottom:8, color:'#1A1613' }}>{item.title}</div>
                  </a>
                  {item.summary && (
                    <div style={{ fontSize:13, color:'#6B655E', lineHeight:1.6, marginBottom:10 }}>{item.summary}</div>
                  )}
                  {item.angle && (
                    <div style={{ background:'#F4F1EC', borderRadius:10, padding:'10px 14px', display:'flex', gap:8, alignItems:'flex-start' }}>
                      <span style={{ fontSize:10, fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase', color:'#9C958B', flexShrink:0, paddingTop:2 }}>Sales angle</span>
                      <span style={{ fontSize:13, fontWeight:600, color:'#3F3A34', lineHeight:1.5 }}>{item.angle}</span>
                    </div>
                  )}
                </div>
                <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ flexShrink:0, padding:'8px 16px', borderRadius:10, border:'1px solid #ECE8E2', fontSize:12, fontWeight:700, color:'#6B655E', textDecoration:'none', whiteSpace:'nowrap' }}>
                  Read →
                </a>
              </div>
            </div>
