...
export default function MobileNav({ onSettings }: { onSettings: () => void }) {
    ...
    <>
      ...
      {menuOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,22,19,.42)', zIndex: 50 }} onClick={() => setMenuOpen(false)}>
          ...
          >
            <div style={{ width: 40, height: 4, background: '#ECE8E2', borderRadius: 2, margin: '0 auto 20px' }} />
            {ALL_SECTIONS.map(section => (
              <div key={section.group}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.18em', textTransform: 'uppercase', color: '#C5BFB6', padding: '8px 24px 4px' }}>{section.group}</div>
                {section.items.map(item => (
                  <Link
                    ...
                    style={{
                      ...
                      color: pathname === item.href ? '#F5552E' : '#1A1613',
