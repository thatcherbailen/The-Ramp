...
export default function SettingsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal title="Settings" onClose={onClose} width={600}>
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #ECE8E2', marginBottom: 24, overflowX: 'auto' }}>
        {TABS.map(t => (
              ...
              {NEWS_CATS.map(cat => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 999,
                    border: '1px solid',
                    borderColor: s.newsCategories.includes(cat) ? '#F5552E' : '#E4DFD8',
                    background: s.newsCategories.includes(cat) ? '#FBE7E0' : '#fff',
                    color: s.newsCategories.includes(cat) ? '#F5552E' : '#9C958B',
                    fontSize: 13,
                    fontWeight: 600,
                    cursor: 'pointer',
                    fontFamily: 'inherit',
      ...
      {tab === 'Nudges' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[
            ...
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', border: '1px solid #ECE8E2', borderRadius: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#9C958B', marginTop: 2 }}>{desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === 'Display' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', border: '1px solid #ECE8E2', borderRadius: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#9C958B', marginTop: 2 }}>Invert the colour scheme — reload to apply</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', border: '1px solid #ECE8E2', borderRadius: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#9C958B', marginTop: 2 }}>Tighter spacing on cards and lists</div>
            </div>
          </div>
          <div>
      ...
      {tab === 'Data' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ background: '#FBF9F6', border: '1px solid #ECE8E2', borderRadius: 14, padding: '16px 20px' }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>Export backup</div>
            <div style={{ fontSize: 13, color: '#6B655E', marginBottom: 14, lineHeight: 1.5 }}>Download a JSON backup of all your data — tasks, calls, jobs, stories, contacts, and settings.</div>
            <button onClick={exportData} className="coral-btn" style={{ height: 40, padding: '0 20px', fontSize: 14 }}>Download JSON backup</button>
          </div>
          <div style={{ background: '#FBE7E0', border: '1px solid #F0D7CF', borderRadius: 14, padding: '16px 20px' }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#C24A24', marginBottom: 6 }}>Reset all data</div>
            <div style={{ fontSize: 13, color: '#C24A24', marginBottom: 14, lineHeight: 1.5, opacity: 0.8 }}>Permanently deletes everything. The seed tasks and prep cards will reload — your own entries will be gone.</div>
            <button onClick={resetAll} style={{ height: 40, padding: '0 20px', fontSize: 14, fontWeight: 700, background: '#C24A24', color: '#fff', border: 'none', borderRadius: 999, cursor: 'pointer', fontFamily: 'inherit' }}>Reset everything</button>
          </div>
        </div>
      )}
    </Modal>
